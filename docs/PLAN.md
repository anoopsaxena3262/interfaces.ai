...
Sandbox double-click handling is in-memory on `Store`, not a ledger unique constraint. Choices and the production remainder: [Replay idempotency](#replay-idempotency).
...
| Product retries | Idempotency unique on `(institution_id, idempotency_key)` in the **same** transaction as the ledger write | Double-click and worker retry. Sandbox map is not this. See [Replay idempotency](#replay-idempotency). |
...
Ledger lock and two-worker safety are **not** built. In-memory replay idempotency is; it does not replace a per-FI write lock. The Later rows for durable ledgers and async workers assume this locking story; they are incomplete without it.

## Replay idempotency

Double-click and HTTP retry must not `apply_transfer` twice for the same attempt. The sandbox implements the *client contract* and an in-process map. Product still needs the unique constraint on the ledger write. This section is what we chose and what we left out.

### What is in the repo

| Choice | Why |
| --- | --- |
| Optional `idempotency_key` on `POST /api/v1/agents/replay` (body, max 128 chars) | Console and bank forms send a UUID. CLI and curl without a key still post every time — a required key would break one-shot `iai replay`. |
| Key lives on `ReplayResult`, not on `TransferIntent` | Intent is the money movement. The key is the retry envelope. |
| Unique in process on `(institution_id, key)` | Two banks can reuse the same UUID string. Product unique should stay scoped the same way. |
| Same key + same fingerprint → return the first `ReplayResult`; do not `add_replay` again; do not call `apply_transfer` | Fingerprint is the **request** intent (ids, amount, currency, memo, actor). Stored `ReplayResult.intent` is last-4 ids and empty memo (compliance Phase 1), so it cannot be the comparison. |
| Same key + different fingerprint → `409` | Matches the Later table. Prevents “reuse this UUID for a different $10 transfer.” |
| Per-slot `threading.Lock` around lookup → run → store | Overlapping double-clicks with the **same** key wait, then see the first result. A process-wide lock would accidentally serialize unrelated posts. |
| Console and bank forms: one UUID per attempt; mint a new key after `2xx`; remint on `409`; keep the key on network failure | Two submits before the first response share a key. A lost response can retry safely. After success, a later click is a new transfer. |
| Reset ledgers does **not** clear keys | Same as discovery/replay/`Store`. After reset, retrying an old key returns the original receipt and does not debit the restored seed. Demo: new key or restart the API. |

`Store.replay_once` in `agents/base.py`. Tests: `test_api.py` (`click-1` same body; `click-2` then different amount → 409).

### What this does not cover (production gap)

Keep the HTTP contract (client-supplied key, same body = replay, different body = 409). Replace the map.

| Gap | Why the sandbox is not enough |
| --- | --- |
| Unique `(institution_id, idempotency_key)` in the **same** DB transaction as `apply_transfer` | Two Uvicorn workers each have their own `Store`. Both can miss, both post, both insert. A dict cannot be the uniqueness mechanism. |
| Restart / deploy | The map dies with the process. A client retry after bounce posts again. |
| Same transaction as the working ledger | If the run row commits and the ledger write does not (or the reverse), retries disagree about whether money moved. Product: one `COMMIT` for key + balances (or equivalent outbox). |
| Two **different** keys still race on `_working` | Idempotency is per attempt, not a per-FI mutex. Overlapping `$40` and `$10` can still lost-update. That is the sandbox-now per-FI lock row, then `SELECT … FOR UPDATE` in product. |
| No TTL / retention | The in-memory dict grows for the life of the process. Product: expire unused keys (e.g. 24h) and keep used keys with the run row. |
| `Idempotency-Key` header | Body field is enough for the demo OpenAPI form. Product APIs usually take the Stripe-style header and reject a missing key on money-movement routes. |
| Scheduled extract ticks | Not implemented. When jobs exist, tick `2026-08-28T16:00Z` + institution is one key; a late retry of that tick must no-op. Same unique table, different `kind`. |
| Key required on every write | Optional key is a laptop escape hatch. Product: require a key on `transfer_replay` (and on job enqueue) so a worker retry cannot invent a second post. |

**Stop:** do not add SQLite “just for keys” while ledgers stay in `_working`. That is a second store that can commit without the money. Durable uniqueness is the same increment as [durable working ledgers](#now-vs-later-sandbox-vs-product).
...
| Same replay key does not post twice | `test_api.py` — same `idempotency_key` returns first `run_id`; different body is 409 |
| Agent canonical omits contact | `test_api.py` — `?view=agent` |
| PAN/SAD ingest is rejected | `test_cardholder.py` — Luhn test PAN; seeds still map |
...
- Docs and pytest (including API 404/422 and replay idempotency)
- Compliance **Phase 1–3**: copies, `GET /canonical?view=agent`, Luhn/SAD ingest guard (`cardholder.py`)

**“Now” does not mean shipped.** Rows tagged sandbox-now in [Now vs later](#now-vs-later-sandbox-vs-product) are *appropriate to build in this repo* (still one process, no Postgres, no Playwright, no LLM). Most of them are **not implemented**. Phase 1–3 compliance, in-memory replay idempotency, and the demo stack above are in code.
...
| Idempotency keys on replay | Double-click / retry must not post twice | Double-submit / retry posts twice | **Sandbox is in the repo** (optional body key, in-memory map, `409` on conflict). **Product:** unique `(institution_id, idempotency_key)` in the **same** transaction as the ledger write; require the key on writes; TTL; scheduled-job keys. See [Replay idempotency](#replay-idempotency). |
...
| --- | --- | --- |
| **In repo** | Code and tests exist | Demo stack + compliance Phase 1–3. |
...
| Compliance Phase 2 (`GET /canonical?view=agent`) | Console and agents do not need email/phone/transactions. Default `/canonical` stays full for schema/debug. `/native` is the portal contract (demo-only unauthenticated). |
| Compliance Phase 3 (Luhn + SAD ingest) | Shared `reject_cardholder_data` on `load_native` and `to_canonical`. Test PAN only in `test_cardholder.py`. **Stop:** no ROC, no encrypt-PAN (Phase 4). |
| API 404 / 422 on unknown institution and bad amount | HTTP contract for the console and OpenAPI, not a product auth story. |
| In-memory replay `idempotency_key` | Console/bank UUID per attempt; `Store.replay_once`; fingerprint on the request intent (not the redacted stored intent). Same key + same body returns the first `ReplayResult`; conflict is `409`. **Stop:** durable unique in the ledger transaction is product. Choices and remainder: [Replay idempotency](#replay-idempotency). |

### Sandbox now — appropriate here, **not implemented**

Suggested order if we extend the sandbox: (1) split hold reasons + status enum, (2) hold on missing paths, (3) hint/contract drift tests, (4) per-FI lock, (5) eval corpus, (6) `schema_version: "1.0.0"` stub.
...
| Durable replay idempotency (unique with the ledger write) | Restart, two workers, and worker retry must not post twice | The in-memory map is the demo. Do not persist keys without the money. Same increment as durable ledgers. [Replay idempotency](#replay-idempotency). |
...
| Honest holds + no PII in `/runs` | **Yes** for copies and agent canonical (Phase 1–2); coarse `POLICY` remains | Sandbox-now: reason split |
...
| Double-click does not post twice | **Partial** (one process, until restart) | Durable unique with the ledger write |
...
| Double-submit / retry posts twice | Client `idempotency_key`; in-memory `(institution, key)` on `Store`. Same body returns the first receipt; different body is `409`. Per-slot lock for overlapping same-key clicks. | **Keep the idea.** Memory map dies on restart; two workers do not share it; reset-ledgers leaves keys while money rewinds. | Unique `(institution, key)` in the same transaction as the post. Require a key on writes. TTL. See [Replay idempotency](#replay-idempotency). |
...
| Snapshot and logs copy more PII than agents need | Phase 1–3: stored copies redacted; `?view=agent`; Luhn/SAD reject at ingest. **Still:** Calloway DDA as `Account.id` in working snapshot; full `/canonical` and `/native` for debug/portals | **Phase 1–3 in repo.** Residual on posting ids and demo `/native`. PCI out of scope unless PAN appears (then ingest fails). | Phase 4 after durable Store. [PLAN-COMPLIANCE.md](PLAN-COMPLIANCE.md). |
